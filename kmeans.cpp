...
class KMEANS
{
	private:
		...
		vector< T > mmin, mmax; // the data set
		int colLen, rowLen; // dimension of vector; number of vectors
		int k;
		vector< vector<T> > centroids;
		typedef struct MinMax // 每一维的最大最小
		{
			T Min;
			T Max;
			MinMax(T min, T max):Min(min),Max(max) {} //序列初始化
		} tMinMax;
		typedef struct Node 
		{
			int minIndex; // the index of each node,属于哪个质心的类别
			double minDist; // 与最近质心的距离
			Node(int idx, double dist):minIndex(idx),minDist(dist) {}
		} tNode;
		vector<tNode> clusterAssignment;
	
		// split line into numbers	
		void split(char *buffer, vector<T> &vec);
		tMinMax getMinMax(int idx);
		void setCentroids(tMinMax &tminmax, int idx);
		void initClusterAssignment();
	...
	while ( clusterChanged )
	{
		...
		for (int i = 0; i < rowLen; ++i)
		{
			...
			for (int j = 0; j < k; ++j)
			{
				...
				if ( distJI < minDist ) // 只取最短距离么
				{
					minDist = distJI;
					minIndex = j;
				}
			}
			if ( clusterAssignment[i].minIndex != minIndex )
			{
				clusterChanged = true;
		...
		for (int cent = 0; cent < k; ++cent)
		{
			...
			for (int i = 0; i < rowLen; ++i)
			{
				if ( clusterAssignment[i].minIndex == cent )
				{
					++cnt;
					// sum of two vectors
					for (int j = 0; j < colLen; ++j)
			...
			for (int i = 0; i < colLen; ++i)
			{
				if ( cnt != 0 )
					vec[i] /= cnt;
				centroids[cent].at(i) = vec[i]; // 质心是虚拟的
			}
		}
		print();
	}
...
template<typename T>
void KMEANS<T>::initClusterAssignment()
{
	tNode node(-1, -1);
	for (int i = 0; i < rowLen; ++i)
	{
...
int main(int argc, char *argv[])
{
	...
	kms.randCent(); // 考虑数据的每一维，随机生成k个中心点
	kms.kmeans();
